...
export default function TermsAndConditions() {
  ...
  useEffect(() => {
    const GotTerms = async () => {
      ...
      const res = await GetWithToken<MyResponse<TermsAndConditions>>({
        router: router,
        url: urlwithQuery,
        token: `${auth.auth.access_token}`,
      });
      ...
      if (res.statusCode === 200) {
        ...
        formik.setFieldValue(
          "outlet_id",
          res.data.outlet ? res.data.outlet.id : "all",
        );
        ...
        formik.setFieldValue(
          "items",
          res.data.terms_and_conditions_items.map((item) => ({
            id: item.id,
            terms_and_conditions_id: item.terms_and_conditions_id,
            label: item.label,
            text: item.text,
          })),
        );
  ...
  const formik = useFormik<TAndCUpdateFormValues>({
    initialValues: {
      id: "",
      outlet_id: "all",
      title: "",
    ...
    onSubmit: async (values) => {
      ...
      const payload: CreateUpdateTAndCDTO = {
        ...
        receipt_icon: values.receipt_icon ?? undefined,
        items: values.items.map((item) => ({
          id: item.id,
          terms_and_conditions_id: item.terms_and_conditions_id ?? undefined,
          label: item.label,
          text: item.text,
        })),
      };
      ...
      if (res.statusCode === 200) {
        toast.success("Success update Terms and Conditions!");
