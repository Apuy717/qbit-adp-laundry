...
interface iModalOutlet {
  showModal: boolean;
  coleModal: () => void;
  onSelected: (outlet: Outlet) => void;
}


export function ModalOutletComponent(props: iModalOutlet) {
  ...
  useEffect(() => {
    ...
    async function GotItems() {
      let urlwithQuery = `/api/outlet/got/forme?is_deleted=false&page=${currentPage}&limit=${10}`;
      if (fixValueSearch.length >= 1) {
        urlwithQuery = `/api/outlet/got/forme?is_deleted=false&page=${currentPage}&limit=${10}&search=${fixValueSearch}`;
      }
  ...
  }, [props.showModal, access_token, router, currentPage, fixValueSearch, refresh])
  ...
  return (
    <Modal isOpen={props.showModal}>
      ...
      h-[80%] w-[95%] md:h-[90%] md:w-[70%]">
          ...
          <div className="h-[80%]">
            <Breadcrumb pageName={"Select Outlet"} />
            ...
            <div className="overflow-y-auto h-full">
              {!loadingSearch && (
                <Table
                  colls={["#", "Name", "Phone Number", "Address"]}
