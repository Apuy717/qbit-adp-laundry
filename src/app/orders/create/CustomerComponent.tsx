...
export function ModalCustomerComponent(props: iModalCustomer) {
  ...
  useEffect(() => {
    ...
    async function GotItems() {
      let urlwithQuery = `/api/customer?page=${currentPage}&limit=${10}`;
      if (fixValueSearch.length >= 1) {
        urlwithQuery = `/api/customer?page=${currentPage}&limit=${10}&search=${fixValueSearch}`;
      }

      const res = await GetWithToken<iResponse<iCustomer[]>>({
        url: urlwithQuery,
        router: router,
        token: `${access_token}`
      })
  ...
  return (
    <Modal isOpen={props.showModal}>
      ...
      h-[80%] w-[95%] md:h-[90%] md:w-[70%]">
          ...
          <div className="h-[80%]">
            <Breadcrumb pageName={"Select Customer"} />
